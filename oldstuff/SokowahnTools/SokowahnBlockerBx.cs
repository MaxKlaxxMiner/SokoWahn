...
namespace Sokosolver.SokowahnTools
{
  ...
  /// </summary>
  public class SokowahnBlockerBx : ISokowahnBlocker
  {
    /// <summary>
    /// merkt sich das Original-Spielfeld
    /// </summary>
    readonly SokowahnRaum basisRaum;
    ...
    struct BlockerFeld
    {
      ...
      {
        if (blockerSortiert) throw new Exception("Kein neuer Eintrag in sortierten Blockern möglich!");
        ...
        if (this.kistenAnzahl == 0)
        {
          ...
          blockerAnzahl = 0;
          blockerData = new int[blockerGröße]; // Platz für ersten Datensatz schaffen
          blockerSortiert = false;
        }

        if (kistenAnzahl != this.kistenAnzahl) throw new Exception("Fehler, Anzahl der Kisten stimmt nicht überein!");

        int p = blockerAnzahl * blockerGröße; // Position des nächsten Datensatzes berechnen

        if (p == blockerData.Length) Array.Resize(ref blockerData, blockerData.Length * 2);

        for (int i = 0; i < kistenAnzahl; i++)
        {
          blockerData[p++] = neuKisten[i];
          blockerData[p++] = 0; // Sprung-Marker anfangs leer lassen
        }
      ...
      {
#if DEBUG
        if (blockerAnzahl == 0) return false; // keine Blocker bekannt
        if (!blockerSortiert) throw new Exception("Blocker nicht sortiert!");
#endif
      ...
      void Sortiere(int pos, int kistenIndex, int anzahl)
      {
        ...
        var tmpData = new int[anzahl * blockerGröße];
        ...
        var posis = Enumerable.Range(0, anzahl).Select(i => i * blockerGröße).ToArray();
    ...
    public SokowahnBlockerBx(string blockerDatei, SokowahnRaum basisRaum)
    {
    ...
    void SammleKistenInit(bool zielVariante)
    {
      ...
      var feldData = basisRaum.FeldData;
      ...
      {
        tmpRaum = new SokowahnRaum(feldData, feldBreite) { KistenAnzahl = suchKistenAnzahl };
    ...
    public bool Next(int limit)
    {
      ...
      switch (status)
      {
        ...
        case BlockerStatus.SammleStartStellungen:
        {
          ...
          if (SammleKistenNext()) return true;
...
.SelectMany(stellung =>
        ...
        {
          limit = (int)Math.Min(limit, prüfListeBöse.SatzAnzahl);
    ...
    long NextSchätzen
    {
      get
      {
        ...
        return (long)(letzteAnzahl / (double)davorAnzahl * letzteAnzahl);
      }
    }
    ...
    public override string ToString()
    {
      ...
      switch (status)
      {
        ...
        case BlockerStatus.SucheVarianten: ausgabe.AppendLine("[" + suchKistenAnzahl + "] - Suche: " + (prüfListe.SatzAnzahl + prüfListeSammler.SatzAnzahl).ToString("#,##0") + " / " + bekannteStellungen.HashAnzahl.ToString("#,##0") + " (" + ((prüfListe.SatzAnzahl + prüfListeSammler.SatzAnzahl) * 100.0 / bekannteStellungen.HashAnzahl).ToString("0.00") + " %" + (NextSchätzen > 0 ? ", " + NextSchätzen.ToString("#,##0") + " max." : "") + ")"); break;
