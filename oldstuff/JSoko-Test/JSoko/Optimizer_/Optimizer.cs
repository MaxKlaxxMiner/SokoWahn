...
{
  /// <summary>
  /// This code is heavily inspired by the code from Sébastien Gouëzel. It's a migrated version of his C++ coding. Hence, a big "thank you" to Sébastien Gouëzel, for inventing the "Vicinity search"
  /// optimization method, and generously sharing information on the method and its implementation. By sharing his ideas and insights on the subject, he has made a significant and lasting 
  /// contribution to the Sokoban game itself, transcending the implementation of the algorithm in this optimizer.
  /// 
  /// This class offers the functionality of optimizing a set of existing solutions. The incantation logic is as follows:
  /// 
  /// The constructor gets passed the most basic data of the optimization job, most notably the board, inside which solutions will be optimized.
  /// The constructor already starts a background thread to identify (computeand collect) a certain kind of deadlock configurations.
  /// 
  /// The real optimization job is started by invocation of the method <code>startOptimizer</code>, which creates a new thread to do the job by invocation of our <code>run</code> method.
  /// 
  /// The optimization can be stopped prematurely by invocation of the method <code>stopOptimizer</code>, which sets a global flag, that is frequently checked by the worker thread.
  /// 
  /// New solutions found by this class are communicated by invocation of the GUI notification callback method <code>newFoundSolution</code>, in the middle of its work, and by
  /// <code>optimizerEnded</code> when the optimizer has completed its work.
  /// 
