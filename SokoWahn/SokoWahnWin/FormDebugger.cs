...
namespace SokoWahnWin
{
  public sealed partial class FormDebugger : Form
  {
    ...
    #region # public static RoomNetwork CreateTestRooms() // Test-Räume anhand eines bekannten Levels erstellen
    /// <summary>
    /// Test-Räume anhand eines bekannten Levels erstellen
    /// </summary>
    ...
    void buttonOptimize_Click(object sender, EventArgs e)
    {
      ...
      foreach (var room in optimizeRooms)
      {
        ...
        if (!scanner.Step2_ScanForward(txt => status("Optimize[" + room.fieldPosis.First() + "]: (3 / 5) scan forward - " + txt))) return;

        if (!scanner.Step3_ScanBackward(txt => status("Optimize[" + room.fieldPosis.First() + "]: (4 / 5) scan backward - " + txt))) return;
    ...
    void buttonFilter_Click(object sender, EventArgs e)
    {
      ...
      //buttonFilter.Text = "ok. (" + (oldValue - newValue).ToString("N0") + ")";
      activeMerge = false;
    }
