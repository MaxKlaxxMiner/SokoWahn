		...
		implements DirectionConstants
{
	...
	ArrayList<IBoardPosition> boardPositionsQueueForward  = new ArrayList<IBoardPosition>(500000);
	ArrayList<IBoardPosition> boardPositionsQueueBackward = new ArrayList<IBoardPosition>(500000);
	...
	// werden darf)
	Board.PlayersReachableSquares playersReachableSquares;
	 ...
	 */
	public final void showNotIdentifiedDeadlockPositions() {
	 ...
	 */
	private final void forwardSearch() {
			...
			for (int boxCounter = -1, boxNo; boxCounter < board.goalsCount; boxCounter++) {
				...
				for (int direction = 0; direction < DIRS_COUNT; direction++) {
					...
					if (oldBoardPosition != null) {
						...
						if (((BoardPosition) oldBoardPosition).getSearchDirection() == SearchDirection.FORWARD) {
	 ...
	 */
	private final void backwardSearch() {
	 ...
	 */
	private final boolean isBoxInTunnel(int boxNo, byte searchDirection) {
		...
		for (byte direction = 0; direction < DIRS_COUNT; direction++) {
			...
			if (searchDirection == BACKWARD_SEARCH) {
				if (board.playersReachableSquaresOnlyWalls
						.isSquareReachable(boxPosition + offset[direction]) == false
						|| board.playersReachableSquaresOnlyWalls
